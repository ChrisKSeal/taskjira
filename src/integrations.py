...
class Integrator:
    def __init__(
        ...
        self.jql_sprint = (
            f'assignee = "{self.jira.current_user}" '
            "and sprint != NULL "
    ...
    def get_keys_by_type(
        ...
        return [
            ...
            if str(issue.get_field("issuetype") == issue_type)
        ]
    ...
    def build_priority_from_parents_and_dependencies(
        ...
        for key, issue in issues.items():
            ...
            if parents[key]:
                issue_priority = max(
                    self.__get_numeric_value_for_priority(
                        issues[parents[key]].fields.priority
                    ),
                    self.__get_numeric_value_for_priority(issue.fields.priority),
                )
            else:
                issue_priority = self.__get_numeric_value_for_priority(
            ...
            if key in rev_deps:
                ...
                for dep in rev_deps[key]:
                    dependencies_priority.append(
                        self.__get_numeric_value_for_priority(
                            issues[dep].fields.priority
                        )
                    )
                    if parents[dep]:
                        dependencies_priority.append(
                            self.__get_numeric_value_for_priority(
                                issues[parents[dep]].fields.priority
                            )
                        )
                if parents[key]:
                    dependencies_priority.append(
    ...
    def __get_numeric_value_for_priority(self, priority: Priority) -> int:
        ...
        return (
            0
            if str(priority).lower() not in priority_dict
            else priority_dict[str(priority).lower()]
        )
    ...
    def process_sprints(
        ...
        return_dict: Dict[str, Optional[List[str]]] = {
            key: [field.name for field in issue.fields.customfield_10020]
            if issue.fields.customfield_10020
            else None
            for key, issue in issues.items()
        }
        ...
        for key, task in task_dict.items():
            ...
            if key in tasks_in_tw:
                ...
                if "depends" in old_task:
                    old_task["depends"] = [
                        self.__get_twtask_key_by_uuid(task_uuid, task_dict)
                        for task_uuid in old_task["depends]"]
                    ]
                    print(old_task)
                ...
                if (
                    "due" in old_task.keys()
                    and "due" in new_task.keys()
                    and old_task["due"] != new_task["due"]
                ):
                ...
                if (
                    ...
                    "depends" in new_task.keys() and "depends" not in old_task.keys()
                ):
    ...
    def __get_twtask_key_by_uuid(
        self, uuid: str, task_dict: Dict[str, TWTask]
    ) -> Optional[str]:
        return next((key for key, task in task_dict.items() if task.uuid == uuid), None)

    def parse_twtask_to_dict(
        self, key: str, task_dict: Dict[str, TWTask]
    ) -> Dict[str, Any]:
    ...
    def push_to_taskwarrior(self, task_dict: Dict[str, TWTask]) -> None:
        ...
        for key, task in task_dict.items():
            ...
            if task.depends:
                tw_entry["depends"] = [
                    self.__get_twtask_uuid_by_key(task_key, task_dict)
                    for task_key in tw_entry["depends"]
                ]
        ...
        for key, task in task_dict.items():
            ...
            if (
                "all" in existing_tasks[key] and task.depends
            ) or "depends" in existing_tasks[key]:
                tw_entry = self.parse_twtask_to_dict(key, task_dict)
